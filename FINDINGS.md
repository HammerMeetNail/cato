...
- Prod: 7 exhausted jobs, including both games above. Two more failed on DNS:
  `dial tcp: lookup images.cato.com ... server misbehaving` — the old
  Postgres import points **2,716 games** at `images.cato.com`, a domain
...
re-source `cato.com` URLs during the 1.1 backfill.
...
- Prod DB (read-only queries via `sqlite3 ?mode=ro` on nas2):
  ...
  - `cover_jobs`: 127 total, 7 exhausted (`attempts=5`), incl. games 194464 &
    175604 (`http status 404`) and 4 DNS failures to `images.cato.com`;
    120 pending, all for games not in any library.
  - 2,716 games have `cover_url` on the dead `images.cato.com` domain.
