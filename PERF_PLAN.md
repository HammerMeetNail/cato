...
(`/volume1/Shared/Cato/data/cato.db`).
...
1. **Purge the non-library backlog** (one-time, app stopped). On `nas2`:
   ...
   $D stop cato
   sqlite3 /volume1/Shared/Cato/data/cato.db \
     "DELETE FROM cover_jobs WHERE game_id NOT IN (SELECT DISTINCT game_id FROM library_items);"
   sqlite3 /volume1/Shared/Cato/data/cato.db "VACUUM;"
   $D start cato
   ```
...
ssh nas2 'sqlite3 "file:/volume1/Shared/Cato/data/cato.db?mode=ro" "PRAGMA journal_mode;"'
...
ssh nas2 'sqlite3 "file:/volume1/Shared/Cato/data/cato.db?mode=ro" "SELECT COUNT(*) FROM cover_jobs;"'
